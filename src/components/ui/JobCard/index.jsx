        ...
        <div className="flex gap-2 flex-wrap">
          {
            job?.jobTags?.map((tag, index) => (
              <div key={index} className="px-2 text-xs py-1 bg-blue-light text-blue">{ tag }</div>
            ))
          }
        </div>
        <div className="grid px-2 py-1 text-sm bg-blue-light text-blue w-fit">{ job?.jobAmount }</div>
      </div>
