      ...
      <div className="flex flex-col gap-20 px-4 md:px-8 lg:px-[8.4375rem] py-20">
        ...
        <div className="flex items-center justify-center w-full mt-4">
          ...
          {blogs && [...Array(paginate({page, num_per_page, data: blogs, total: blogs?.length})?.pages).keys()]?.map((page_num) => 
            <span key={page_num+1} onClick={() => changePage(page_num+1, setPage)} className={`${page===page_num+1 && "bg-green text-white"} font-medium w-6 h-6 text-sm text-black grid place-items-center cursor-pointer`}>1</span>
